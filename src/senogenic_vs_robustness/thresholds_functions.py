...
def load_results(pkl_filename):
    ...
    """
    pkl_file = results_path + pkl_filename
    try:
...
def plot_median_max_plane(
    ...
    Args:
        pkl_file: Pickle filename in results folder (e.g. 'param_variation_results.pkl').
...
def plot_max_lifespan(study_type='variation', ignore_kappa=True, ax=None, new_figure=True, from_t=0, pkl_file=None, normalized=False, **kwargs):
    ...
    Args:
        ...
        pkl_file (str): Optional filename of pkl file in results folder (filename only, not full path).
    ...
    # Load results
    if pkl_file is None:
        pkl_file = os.path.join('results', default_pkl)
