"""
Joint SR fit for USA and Sweden 2019 period HMD curves, focused on ages 65-100.
...
RUN_LABEL = "joint_shared_eta_beta_epsilon_65_100_n100k_tail90_fit_2019"
...
METHODS_PATH = OUTPUT_DIR / "joint_shared_eta_beta_epsilon_65_100_n100k_tail90_methods.md"
...
def fit_parameters(data_by_country: dict[str, CountryData]) -> tuple[np.ndarray, float]:
    starts = [
        np.array([0.10, 0.08, 0.03, 0.05, -0.64, 0.00, -0.30]),
        np.array([0.10, 0.08, 0.03, 0.08, -0.70, 0.00, -0.30]),
        np.array([0.12, 0.08, 0.02, 0.05, -0.58, 0.02, -0.28]),
    ]
...
def plot_results(results: dict[str, object], data_by_country: dict[str, CountryData]) -> None:
    ...
    fig.suptitle(
        f"Joint SR fit focused on ages {FOCUS_START}-{FOCUS_END}: "
        "shared eta, beta, epsilon"
    )
    fig.tight_layout()
...
Extra hazard weight was applied to ages \\(a \\ge 85\\), with stronger weight
for ages \\(a \\ge 90\\). In this tail-focused run, Sweden also received an
additional hazard weight over ages 88-96 because the age-90 Sweden hazard was
the main visual target for refinement.
