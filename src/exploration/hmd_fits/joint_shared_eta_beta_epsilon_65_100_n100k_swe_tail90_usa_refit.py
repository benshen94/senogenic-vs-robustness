"""
Hybrid SR fit for USA and Sweden 2019 period HMD curves, focused on ages 65-100.

Constraint:
- eta, beta, epsilon are fixed from the Sweden-tail fit.
- Sweden Xc and Xc heterogeneity are fixed from the Sweden-tail fit.
- Only USA Xc and USA Xc heterogeneity are refit here.
...
RUN_LABEL = "joint_shared_eta_beta_epsilon_65_100_n100k_swe_tail90_usa_refit_2019"
SUMMARY_PATH = OUTPUT_DIR / f"{RUN_LABEL}_summary.json"
PLOT_PATH = OUTPUT_DIR / f"{RUN_LABEL}.png"
CI_PATH = OUTPUT_DIR / f"{RUN_LABEL}_ci.csv"
METHODS_PATH = OUTPUT_DIR / "joint_shared_eta_beta_epsilon_65_100_n100k_swe_tail90_usa_refit_methods.md"
...
FIT_PARAMETER_DIMS = (5, 6)
FIT_PARAMETER_NAMES = ("usa_xc_log2", "usa_xc_std_log2")
FIXED_ANCHOR_VECTOR = np.array([0.12, 0.08, -0.06, 0.05, -0.50, 0.02, -0.18])
...
def residual_vector(
    ...
    for index, country in enumerate(COUNTRIES):
        ...
        hazard_weights[hazard_ages >= 80] = 1.4
        hazard_weights[hazard_ages >= 90] = 2.2
        ...
        if include_survival:
            ...
            residuals.append(np.sqrt(16.0) * survival_residual)
...
def coordinate_search(
    ...
    for step_size in step_sizes:
        ...
        while improved:
            ...
            for dim in FIT_PARAMETER_DIMS:
                name = PARAMETER_NAMES[dim]
...
def fit_parameters(data_by_country: dict[str, CountryData]) -> tuple[np.ndarray, float]:
    starts = [
        FIXED_ANCHOR_VECTOR,
        np.array([0.12, 0.08, -0.06, 0.05, -0.50, 0.00, -0.30]),
        np.array([0.12, 0.08, -0.06, 0.05, -0.50, -0.05, -0.12]),
        np.array([0.12, 0.08, -0.06, 0.05, -0.50, 0.04, -0.24]),
    ]

    candidates = []
    for start in starts:
        print("Fit start", start.tolist())
        vector, score = coordinate_search(data_by_country, start, step_sizes=(0.08, 0.04, 0.02, 0.01))
...
def metrics_for_country(data: CountryData, fit: dict[str, np.ndarray]) -> dict[str, float]:
    ...
    age_90_mask = (data.ages_hazard >= 88) & (data.ages_hazard <= 94)
    age_90_log_error = np.log(fit["hazard"][age_90_mask]) - np.log(data.hazard[age_90_mask])
    ...
    return {
        ...
        "hazard_log_rmse_88_94": float(np.sqrt(np.mean(age_90_log_error**2))),
        "hazard_median_bias_88_94": float(np.median(age_90_log_error)),
...
def jacobian_ci(
    ...
    jacobian = np.zeros((base_residuals.size, len(FIT_PARAMETER_DIMS)), dtype=float)

    for col, dim in enumerate(FIT_PARAMETER_DIMS):
        name = PARAMETER_NAMES[dim]
        plus = vector.copy()
        minus = vector.copy()
        plus[dim] += delta
        ...
        jacobian[:, col] = (plus_residuals - minus_residuals) / width
    ...
    for value, dim in zip(se_log2_valid, np.asarray(FIT_PARAMETER_DIMS)[valid_columns]):
        se_log2[dim] = value
...
def build_ci_rows(vector: np.ndarray, se_log2: np.ndarray) -> list[dict[str, float | str]]:
    ...
    for idx, name in enumerate(natural_names):
        if idx not in FIT_PARAMETER_DIMS:
            continue
...
def plot_results(results: dict[str, object], data_by_country: dict[str, CountryData]) -> None:
    ...
    fig.suptitle(
        f"Hybrid SR fit focused on ages {FOCUS_START}-{FOCUS_END}: "
        "Sweden fixed from tail90, USA refit"
    )
    fig.tight_layout()
...
def write_methods_markdown(ci_rows: list[dict[str, float | str]]) -> None:
    METHODS_PATH.write_text(
        f"""# Hybrid SR Fit Methods: Sweden Tail90 Anchor and USA Refit
...
parameters \\(\\eta\\), \\(\\beta\\), and \\(\\epsilon\\) were fixed from the
Sweden-tail fit and then held identical for USA and Sweden. Sweden's
\\(X_c\\) and fractional \\(X_c\\) heterogeneity were also fixed from the
Sweden-tail fit. In this hybrid refit, only USA \\(X_c\\) and USA fractional
\\(X_c\\) heterogeneity were optimized. The country-specific external Makeham
term, \\(h_{{ext}}\\), was fixed from a Gamma-Gompertz-Makeham fit to each
country's HMD hazard curve and was not optimized as an SR parameter.
...
optimization. The objective minimized USA-only weighted residuals over ages
{FOCUS_START}-{FOCUS_END}. Sweden was simulated for reporting but did not
contribute to the USA refit objective. Hazard residuals were computed on the log scale:

\\[
r_h(a) = \\log h_{{SR}}(a) - \\log h_{{HMD}}(a).
\\]

Extra USA hazard weight was applied to ages \\(a \\ge 80\\), with stronger weight
for ages \\(a \\ge 90\\). USA survival residuals over the same age window were
included as a regularizer so that the fitted hazards also reproduced cumulative
survival:
...
Approximate 95% confidence intervals for the two USA-refit parameters were
estimated from the local curvature of the fitted residual surface in
log2-parameter space. Around the final parameter vector, each fitted USA
parameter was perturbed by \\(\\Delta = 0.035\\) in log2 units, using common
random seeds for the plus and minus simulations. A numerical Jacobian \\(J\\) of
the residual vector was computed by central differences.
...
def build_summary(
    ...
    return {
        "constraint": "eta, beta, epsilon and Sweden Xc/heterogeneity fixed from tail90; USA Xc and heterogeneity refit",
        "fixed_anchor_log2": FIXED_ANCHOR_VECTOR.tolist(),
        "refit_parameter_names": FIT_PARAMETER_NAMES,
