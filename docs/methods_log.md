...
- Coupled \(a+b\): sample \(b\), then set \(a\) on the Strehler-Mildvan compensation line,
  \[
  a_i=a_0\exp[-105(b_i-b_0)].
  \]
...
Fedichev-Gruber heterogeneity rows are reused from `results/tables/supp_model_comparison_max_lifespan.csv` when available. If those rows are missing, they are regenerated directly in the figure script with the legacy notebook model equations:
...
Gompertz shape-response data are regenerated by `make_supp_model_comparison.py` and saved to `results/gamma_factor_sweep.pkl`. The baseline is `payload["results"]["baseline"][0]`, and all points are normalized as:
...
For the coupled Gompertz slope-intercept curve, factors \(q\) are applied as:
...
Gompertz and Fedichev-Gruber shape-response factor values are:

\[
0.6,0.7,\ldots,1.4.
\]

Fedichev-Gruber shape-response data are loaded from `results/fedichev_model_steepness_longevity_data.pkl`.
