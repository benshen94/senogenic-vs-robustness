...
RUN_LABEL = "joint_shared_eta_beta_epsilon_65_100_n100k_fit_2019"
...
METHODS_PATH = OUTPUT_DIR / "joint_shared_eta_beta_epsilon_65_100_n100k_methods.md"
...
def residual_vector(
    ...
    for index, country in enumerate(COUNTRIES):
        ...
        hazard_weights[hazard_ages >= 85] = 2.2
        hazard_weights[hazard_ages >= 90] = 3.0
        if country == "SWE":
            sweden_tail_mask = (hazard_ages >= 88) & (hazard_ages <= 96)
            hazard_weights[sweden_tail_mask] *= 1.6
        ...
        if include_survival:
            ...
            residuals.append(np.sqrt(6.0) * survival_residual)
...
def fit_parameters(data_by_country: dict[str, CountryData]) -> tuple[np.ndarray, float]:
    starts = [
        np.array([0.08, 0.08, 0.07, 0.05, -0.67, 0.00, -0.44]),
        np.array([0.08, 0.08, 0.07, 0.08, -0.72, -0.02, -0.44]),
        np.array([0.10, 0.08, 0.05, 0.05, -0.62, -0.04, -0.36]),
    ]
