...
from senogenic_vs_robustness.paths import FIGURES_DIR, HMD_DATA_DIR, RESULTS_DIR as PROJECT_RESULTS_DIR
...
FIGURE_DIR = FIGURES_DIR / "Figure4"
RESULTS_DIR = PROJECT_RESULTS_DIR / "figure4"

OUTPUT_PNG = FIGURE_DIR / "sweden_period_both_conditional_age0_contours_1900_2020.png"
OUTPUT_PDF = FIGURE_DIR / "sweden_period_both_conditional_age0_contours_1900_2020.pdf"
OUTPUT_CSV = RESULTS_DIR / "sweden_period_both_conditional_age0_contours_1900_2020.csv"
...
def build_contours(life_table: pd.DataFrame) -> pd.DataFrame:
    ...
    for year, year_data in life_table.groupby("year"):
        ...
        lx_at_start = year_data.loc[year_data["age"] == FROM_AGE, "lx"]
        if lx_at_start.empty:
            continue

        year_data["conditional_survival"] = year_data["lx"] / float(lx_at_start.iloc[0])
...
def plot_contours(contours: pd.DataFrame) -> None:
    ...
    fig.subplots_adjust(left=0.14, right=0.955, bottom=0.16, top=0.88)
    fig.savefig(OUTPUT_PNG, dpi=280)
